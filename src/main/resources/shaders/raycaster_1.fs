...
{       
    ...
    //vec3 rayDirection = normalize(vertexOutModel - effectiveEyePos);
    ...
    for (int i = 0; i < numSamples && travel > 0.0; ++i, pos += stepValue, travel -= stepSize) {
        density = texture(volumeTexture, pos - translation).x * densityFactor;
        //density = texture(volumeTexture, mat3(inverse(model)) * pos - translation).x * densityFactor;
